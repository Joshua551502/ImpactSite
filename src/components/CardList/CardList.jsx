...
export default function CardList() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.keyCode === 39) {
        setFlippedCards(Array(12).fill(true)); // Flip all cards on Right Arrow
      }
      if (e.keyCode === 37) {
        setFlippedCards(Array(12).fill(false)); // Reset on Left Arrow
      }
    };
  ...
  return (
    <div className={styles.cardGrid}>
      {Array.from({ length: 12 }).map((_, index) => (
        <div
          key={index}
          className={`${styles.page} ${flippedCards[index] ? styles.flip : ""}`}
